...
export default class PracticeScene extends Component {
  ...
  componentWillMount () {
    rvDB.showDataItemRows(this.props.navigation.state.params.db).then(([results, lastYLocation]) => {
      if (results === null) {
  ...
  componentDidMount () {
    BackHandler.addEventListener('PracticeSceneBackPress', this._practiceSceneBackPressHandler)
  }

  componentWillUnmount () {
    BackHandler.removeEventListener('PracticeSceneBackPress', this._practiceSceneBackPressHandler)
  }
    ...
    return (
      <View style={{flex: 1}} onLayout = {this._checkDeviceWidth}>
        <ToolbarAndroid
          ...
          title={this.props.navigation.state.params.db}
          titleColor="white"
          ...
          <SwipeListView
              ...
              <View style={styles.rowFront}>
                {
                  this.state.state !== 'initDone' ? (
                    ...
                    <ParsedText
                      ...
                      parse={[
                        // ParsedText not support LongPress
                        {pattern: /\w+/, onPress: this._handleWordPress}
                      ]}
                    >
                      {rowData.item}
                    </ParsedText>
            ...
            renderHiddenRow={(rowData, secdId, rowId, rowMap) => (
              <TouchableOpacity
                ...
                onPress={this._delete.bind(this, secdId, rowId, rowMap)}
              >
                <View style={{width: 66}}>
                  <Image
            ...
            disableLeftSwipe={this.state.state !== 'initDone'}
            ...
            this.lastYLocation !== 0 && this.state.state === 'initDone' && (
              <View style={styles.overlay}>
                ...
                <TouchableOpacity onPress={() => {
                  ...
                  >
                    <Image
                      source={{uri: 'ic_bookmark_border_white_36dp'}}
  ...
  _delete (secdId, rowId, rowMap) {
    ...
    rvDB.deleteDataItem(this.props.navigation.state.params.db, rowId).then(() => {
      data.splice(rowId, 1)
  ...
  _exitPracticeScene () {
    ...
    rvDB.saveDataItemBookmark(this.props.navigation.state.params.db, leaveLocation).then(() => {
      // this.props.navigator.pop()
      this.props.navigation.goBack()
    }).catch((e) => {
      ToastAndroid.show('' + e, ToastAndroid.SHORT)
  ...
  _practiceSceneBackPressHandler () {
    ...
    if (currentRoute === 'PracticeScene') {
      this._exitPracticeScene()
      return true
    }
  }
}
