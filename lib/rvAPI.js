...
var rvDB = {
  ...
  addData: async function (dataItemName, contents, reg) {
    try {
      ...
      let isDataItemNameExist = value !== null
      ...
      if (isDataItemNameExist && lastContent === null) {
        // 这里应该是历史遗留问题，当时没有设置lastContent
        // 如果content的内容完整的包含之前导入的数据，如何办？特别是rows里已经有数据给删除
        // 如果content的内容只包含了一部分之前导入的数据，如何办？
        // 如果content的内容完全和之前导入的数据不相同，如何办？
        // 重点是处于现在的条件，如何作出以上三种情况的判断？

        // there are no dataItemNameLastContent before, so we have to built one
        ...
        // lastRowLocation不可能是-1吧？
        if (lastRowLocation === -1) {
          throw Error('从导入文档中找不到当前同名数据的最后词条，无法判断前后文档是否同源')
        }
      ...
      // 如何保证lastContent和contents都是出入同一种导出方式？不同方式的混用会导致词条重复导入
      ...
      if (incressRows > 0) {
        ...
        throw Error('新增词条为0，或是此次导入文件较上次导入没有新增内容，或者是程序无法提取内容')
      }

      if (isDataItemNameExist) {
        // why isDataItemNameExist, we return undefined
  ...
  addJsonData: async function (dataItemName, contents, checkPropertyArray) {
    try {
      ...
      if (!checkPropertyArray.every((propertyValue) => contents.hasOwnProperty(propertyValue))) {
        throw Error('该JSON文档不符合RoteVocab的数据规范')
      }
      ...
      if (value === null) {
        ...
        if (presentDataItem.totalLength > contents.totalLength) {
          throw Error('旧文档无法覆盖新文档，但你可选择用新的名字来创建该文档。')
        } else {
          ...
          if (lastContentFromJSON !== null) {
            ...
            if (presentLastContent === null) {
              ...
              throw Error('json文档为旧格式，无法替换新格式数据')
            }
          }
        }
      }
  ...
  }, // should check the new name did not exist before executing this function
  changeItemName: async function (dataItemName, nextDataItemName) {
    try {
      // 换dbNameCollection和相应的dataItem,以及dataItemLastContent
      let [dbNameCollection, dataItem, dataItemLastContent] = await Promise.all([
        AsyncStorage.getItem('dbNameCollection'),
        AsyncStorage.getItem(dataItemName),
        AsyncStorage.getItem(dataItemName + 'LastContent')
      ])

      dbNameCollection = JSON.parse(dbNameCollection)
      dbNameCollection[nextDataItemName] = dbNameCollection[dataItemName]
      delete dbNameCollection[dataItemName]

      await AsyncStorage.multiSet([
        ['dbNameCollection', JSON.stringify(dbNameCollection)],
        [nextDataItemName, dataItem],
        [nextDataItemName + 'LastContent', dataItemLastContent]
      ])
...
var bookAPI = {
  ...
  getBookID: async function (bookName, count = 20) {
    try {
      let fields = 'id,title,image'
      let res = await fetch(doubanBookAPIUrl + 'search?q=' + bookName.split(' ').join('%2B') + '&count=' + count + '&fields=' + fields)

      if (!res.ok) {
        throw Error(res.status + ', 获取图书ID时遇到网络故障')
      }
  ...
  getBookCoverUrl: async function (bookID) {
    try {
      ...
      if (!res.ok) {
        throw Error(res.status + ', 获取图书封面时遇到网络故障')
      }
      let body = await res.text()
